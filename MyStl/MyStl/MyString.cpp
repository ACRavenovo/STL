...
public:
	...
	String(const char* str = "") // 缺省一个没有字符的字符串，编译器会将其对待为只有/0
		:_size(strlen(str)),
		 _capacity(_size), // 如果你直接赋值_size+1, 那后面堆里开出的空间就会有可存
		 _str(new char[_capacity + 1]) // 给最后的/0留个空间，因为strlen读到/0就停了
	{
		strcpy(_str, str);
	}
	...
	*/
	...
	String& operator + (const char* str) {
		...
		int strSize = strlen(str);
		if (_capacity - _size < strSize) {
			Ret.SpaceExpending(_capacity + strSize);
		}
		strcpy(Ret._str + Ret._size, str);
		Ret._size = strSize + Ret._size;
		return Ret;
	}
...
int main() {
	...
	s4 += "In";
	s4 += "Initial string";
	std::cout << s4.find(s0.c_str(), 3) << '\n';
	std::cout << s4.find(3, 'a') << '\n';
	s4.resize(10);
	std::cout << s4.size() << '\n';
	s0.reserve();
	std::cout << s0.capacity() << '\n';
	s0 += 'a';
	std::cout << s0 << '\n';
	s0 += s4.c_str();
	std::cout << s0 << '\n';
